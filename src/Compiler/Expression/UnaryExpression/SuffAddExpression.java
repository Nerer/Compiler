 ...
 */
public class SuffAddExpression extends UnaryExpression {
    public SuffAddExpression(Type type, boolean leftValue, Expression expression) {
        super(type, leftValue, expression);
    }

    public static Expression getExpression(Expression expression) {
        if (expression.type instanceof IntType && expression.leftValue) {
            return new SuffAddExpression(Table.myInt, false, expression);
        }
        throw new Error();
    }
    ...
    public void emit(List<Instruction> instructions) {
        ...
        if (expression.operand instanceof Address) {
            ...
            instructions.add(AddInstruction.getInstruction(expression.operand, expression.operand, new Immediate(1)));

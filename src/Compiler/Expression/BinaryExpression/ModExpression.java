...
import Compiler.IR.ArithmeticIR.Binary.ModInstruction;
...
public class ModExpression extends BinaryExpression {
    public ModExpression(Type type, boolean leftValue, Expression lhs, Expression rhs) {
    ...
    public static Expression getExpression(Expression lhs, Expression rhs) {
        if (lhs.type instanceof IntType && rhs.type instanceof IntType) {
            return new ModExpression(Table.myInt, false, lhs, rhs);
        }
        throw new Error();
    }
    ...
    public void emit(List<Instruction> instructions) {
        ...
        instructions.add(ModInstruction.getInstruction(operand, lhs.operand, rhs.operand));
    }
}

...
public class MulExpression extends BinaryExpression {
    public MulExpression(Type type, boolean leftValue, Expression lhs, Expression rhs) {
    ...
    public static Expression getExpression(Expression lhs, Expression rhs) {
        if (lhs.type instanceof IntType && rhs.type instanceof IntType) {
            return new MulExpression(Table.myInt, false, lhs, rhs);
        }
        throw new Error();
    }
    ...
    public void emit(List<Instruction> instructions) {
        ...
        instructions.add(MulInstruction.getInstruction(operand, lhs.operand, rhs.operand));
    }
}

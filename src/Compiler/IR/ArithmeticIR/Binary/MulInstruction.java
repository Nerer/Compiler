...
public class MulInstruction extends BinaryInstruction {
    public MulInstruction(VRegister target, Operand source1, Operand suorce2) {
    ...
    public static Instruction getInstruction(Operand target, Operand source1, Operand source2) {
        if (target instanceof VRegister) {
            return new MulInstruction((VRegister)target, source1, source2).update();
        }
        throw new Error();
    }
    ...
    public String toString() {
        return String.format("%s = mul %s %s", target, source1, source2);
    }
}

 ...
 */
public class LogicalAndInstruction extends BinaryInstruction {
    public LogicalAndInstruction(VRegister target, Operand source1, Operand suorce2) {
    ...
    public static Instruction getInstruction(Operand target, Operand source1, Operand source2) {
        if (target instanceof VRegister) {
            return new LogicalAndInstruction((VRegister)target, source1, source2).update();
        }
        throw new Error();
    }
    ...
    public String toString() {
        return String.format("%s = land %s %s", target, source1, source2);
    }
}

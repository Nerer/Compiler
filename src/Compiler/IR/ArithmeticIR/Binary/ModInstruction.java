...
public class ModInstruction extends BinaryInstruction {
    public ModInstruction(VRegister target, Operand source1, Operand suorce2) {
    ...
    public static Instruction getInstruction(Operand target, Operand source1, Operand source2) {
        if (target instanceof VRegister) {
            return new ModInstruction((VRegister)target, source1, source2).update();
        }
        throw new Error();
    }
    ...
    public String toString() {
        return String.format("%s = mod %s %s", target, source1, source2);
    }
}
